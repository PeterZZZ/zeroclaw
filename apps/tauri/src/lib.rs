...
/// Track the last-known input monitoring status so we can detect revocations
/// when the app resumes from the background.
#[cfg(target_os = "macos")]
static INPUT_WAS_GRANTED: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(false);

/// Check input monitoring status on resume. If it was previously granted but
/// is now denied, emit an `input-monitoring-lost` event to all webviews.
#[cfg(target_os = "macos")]
fn check_input_on_resume<R: tauri::Runtime>(app: &tauri::AppHandle<R>) {
    let current = crate::macos::permissions::check_input_monitoring() == "granted";
    let was_granted = INPUT_WAS_GRANTED.load(std::sync::atomic::Ordering::Relaxed);

    if was_granted && !current {
        let _ = app.emit("input-monitoring-lost", ());
    }

    INPUT_WAS_GRANTED.store(current, std::sync::atomic::Ordering::Relaxed);
}

/// Initialize input monitoring tracking — snapshot the current status.
#[cfg(target_os = "macos")]
fn init_input_tracking() {
    let granted = crate::macos::permissions::check_input_monitoring() == "granted";
    INPUT_WAS_GRANTED.store(granted, std::sync::atomic::Ordering::Relaxed);
}
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Snapshot initial input monitoring status for revocation detection.
            #[cfg(target_os = "macos")]
            init_input_tracking();
        ...
        .run(|app, event| {
            match event {
                ...
                // Re-check input monitoring when the app returns to the foreground.
                RunEvent::Resumed => {
                    #[cfg(target_os = "macos")]
                    check_input_on_resume(app);
                }
                _ => {}
            }
        });
